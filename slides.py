...
class c_Methodology(Slide, ThreeDScene):
    def construct(self):
        ...
        enron_log_graph = BarChart(values=logify(bins['lin']['enron']),
                                y_range=[0, 8],
                                y_length=5,
                                x_length=7,
                                bar_colors=[PURPLE_D],
        ...
        contracts_log_graph = BarChart(values=logify(bins['lin']['contracts']),
                                y_range=[0, 8],
                                y_length=5,
                                x_length=7,
                                bar_colors=[BLUE_D],
        ...
        self.next_slide()
        self.play(map(FadeOut, [three_grids, stack, clues_text, clues_box, total_questions, total_files, total_vaults]))
...
class f_Regression(Slide):
    ...
    #         data = json.load(infile)
    #     stats_dict = data['stats_dict'] ; weighted_fit = data['weighted_fit']
    ...
    def construct(self):
        ...
        for vendor in vendors:
            self._make_vendor_slide(vendor, stats_dict[vendor], weighted_fit.get(vendor), corp_colors)
    ...
    def _fitting(self):
        ...
        exp_eq = MathTex(r"f(x) = a^x").shift(LEFT*2)
    ...
    def _make_vendor_slide(self, vendor, vendor_dict, fit, corp_colors):
        ...
        self.play(Write(title))
        self.play(Create(plot_axes), Create(plot_labels), Create(tick_labels), run_time=3)
        self.play(Create(lines), Create(legend), run_time=5)
        self.next_slide()
        if fit_line:
...
class g_Conclusion(Slide):
    def construct(self):
        ...
        q2 = Text('2. Does Number of Files Impact \nPerformance?', font_size=24).shift(DOWN*1).shift(LEFT*2)
        a2 = Text('Yes regarding Lexis\nand Westlaw, no regarding\nHarvey', font_size=24).shift(DOWN*0.5).shift(RIGHT*2)
