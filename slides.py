...
#         bullets = VGroup(map(lambda x: MarkupText(x, font_size=18), bullets)).arrange(DOWN, aligned_edge=LEFT, buff=0.5)
...
#         uploaded.arrange(DOWN, aligned_edge=LEFT, buff=0.5).shift(DOWN*2.5).shift(RIGHT*3.75)
...
#         self.play(TransformMatchingShapes(tukey_full, tukey_simple))
#         self.next_slide()
#         self.play(TransformMatchingShapes(tukey_name, tukey_abr))
...
#         self.next_slide()    
#         self.play(TransformMatchingShapes(lin_title, exp_title))
#         self.play(TransformMatchingShapes(lin_eq, exp_eq))
#         self.play(TransformMatchingShapes(lin_graph, exp_graph))
#         self.next_slide()
#         self.play(TransformMatchingShapes(exp_title, pop_title))
#         self.play(TransformMatchingShapes(exp_eq, pop_eq))
#         self.play(FadeIn(pop_graph), TransformMatchingShapes(lin_y_lab, pop_y_lab))
#         self.next_slide()
...
class g_Conclusion(Slide):
    def construct(self):
        self._further()

        conc = Text('Conclusions', font_size=36)
        ...
        q2 = Text('2. Does Number of Files Impact\nPerformance?', font_size=24).shift(DOWN*1).shift(LEFT*2.6)
    ...
    def _further(self):
        def draw_box(text, color=WHITE, buff=0.2):
            return SurroundingRectangle(text, color=color, buff=buff, fill_opacity=0)
        discussion_title = VGroup(Text('Discussion', font_size=36))
        discussion_title += draw_box(discussion_title)
        self.play(Write(discussion_title[0]), Create(discussion_title[1]))
        self.play(discussion_title.animate.shift(UP*3.5))
                  
        parallelization = Text('1. Parallelization')
        serial = VGroup(map(lambda x: Text(x, font_size=18), ['• Fixed compute requirement', 
                         '• Low overhead', 
                         '• Necessary for dependant conditions'])).arrange(DOWN, aligned_edge=LEFT).shift(LEFT*2).shift(UP*1)
        ...
        comps = VGroup([Rectangle(height=0.5, width=0.5, fill_opacity=1, color=ORANGE).move_to([0, 2-(0.7*i),0]) for i in range(8)])
        self.play(TransformMatchingShapes(comp, comps)) 
        self.play(dots.animate.shift(RIGHT*5))
        self.next_slide()
